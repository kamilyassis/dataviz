...
def render_aba_doc():
    st.header("Entenda Melhor Nosso Projeto =)")
    ...
    with col_texto1:
        ...
        st.success(
            ...
            "piores em números absolutos), a aplicação consome a **API do IBGE** para baixar dados do "
        ...
        st.write(
            "O mapa coroplético possui navegação interativa e zoom, respondendo ativamente à seleção de métricas personalizavéis."
            "Na aba de análise, há dois gráficos que rankeam os 10 municípios com maior incidência, além de uma matriz de risco (scatter plot) para correlação bivariada entre prevenção e urgência."
        )

    with col_texto2:
        st.subheader("4. Principais Conclusões")
        st.markdown("""
